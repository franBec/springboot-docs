...
title: <Sentence case string>
---
```

The sidebar is autogenerated from the `docs/` directory structure. Do not modify `sidebars.ts`.

#### Frontmatter Description for Excerpt Override

Docusaurus generates excerpts from the first paragraph of content. When a document starts with `<GithubCommitsInfo`, the component spans multiple lines with attributes, so there is no closing `>` on the same line. This opening tag passes through excerpt filters and shows as "<GithubCommitsInfo (truncated)" in card previews and search results.

To fix this, add an explicit `description` field to the frontmatter. This overrides the auto-generated excerpt:
...
Use this pattern for any document that starts with a multi-line component where the opening tag would leak into the excerpt.
