...
In the [previous document](./software-architecture) we looked at how architectural patterns like Clean and Hexagonal Architecture organize code. Now let's zoom out: there are several ways to structure an entire application, and each approach has its own style, benefits, and challenges.
