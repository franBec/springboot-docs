...
- `test/`: Preconfigured for JUnit tests (More about this in the [Unit Testing section](/testing/about-testing)). It has a skeleton test class `SpringJavaApplication.java` that verifies the app context loads.
...
2. Gets the films information from a [Sakila](/prior-recommended-knowledge/sakila-sample-database) [H2 database](https://h2database.com/html/main.html).
...
This document covered the main class rule, the default project structure Spring Boot generates, and common architectural patterns like Clean and Hexagonal architecture. The next step is generating an actual project using Spring Initializr.
