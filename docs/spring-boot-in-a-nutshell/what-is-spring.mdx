...
Before understanding Spring Boot, you need to know its parent: [Spring Framework](https://spring.io/projects/spring-framework). Spring is a powerful, open-source application framework for Java that simplifies enterprise-level development.

Its core philosophy revolves around:

- **Inversion of control (IoC)**: Let the framework manage object creation and dependencies (via Dependency injection).
  - If this doesn't make sense don't worry. A more detailed explanation is on the next document.
