...
2. **UserController->>UserServiceImpl:** The `UserController` calls the `findAll()` method on the injected `UserService` instance (which is implemented by `UserServiceImpl`).
