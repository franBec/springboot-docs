...
description: Generate JPA entity classes at build time directly from an existing SQL schema instead of writing them by hand.
---
...
In the [previous document](./how-spring-talks-to-databases) we looked at how Spring Boot talks to a database through JDBC, Hibernate, and Spring Data JPA. Now we flip the direction: instead of writing entity classes by hand and letting Hibernate generate the schema, we start from an existing SQL schema and generate the entity classes at build time.
...
- **Runtime and compile dependencies** add the standard Spring Data JPA and H2 jars your application needs at runtime.
...
After running `./gradlew generateEntities` (or any task that depends on it, like `build`), you'll find entity source files under `build/generated/sources/hibernate/<group>/<name>/generated/entity/`.
...
[How Spring Talks to Databases](./how-spring-talks-to-databases#schema-migrations) covers code-first vs. database-first approaches in detail. The short version: code-first is convenient for local development, database-first (Flyway, Liquibase) is safer for production.
