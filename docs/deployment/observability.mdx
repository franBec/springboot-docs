...
sidebar_position: 2
title: Observability
description: Observability is about understanding your application in production through tracing, logs, and metrics without adding print statements and redeploying.
---
...
3. **Visualization (The Dashboard)**: You use a visualization tool (like Grafana) to pull the stored data from the backends and present it in cohesive, readable dashboards.
...
**[Grafana](https://grafana.com/)** is the "single pane of glass" that unifies all three data types. It is a web-based visualization platform that connects to multiple telemetry backends simultaneously.
...
<Admonition type="info" title="Real-World Setups Look Different">
  ...
  providers like [Datadog](https://www.datadoghq.com/) or [Grafana
  Cloud](https://grafana.com/products/cloud/). Bundling everything into a single
...
<Admonition type="warning" title="Grafana Environment Variables">
  Grafana expects the environment variables <code>GF_SECURITY_ADMIN_USER</code>{' '}
  and <code>GF_SECURITY_ADMIN_PASSWORD</code> to be set. Make sure to define
