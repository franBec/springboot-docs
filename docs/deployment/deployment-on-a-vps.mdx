...
description: Deploying to your own VPS with Coolify gives you more control and avoids the cold start limitations of free PaaS tiers.
---
...
<GithubCommitsInfo
  commits={[
    {
      name: 'Coolify + GitHub Actions CI/CD Setup',
      url: 'https://github.com/franBec/springboot-demo-projects/commit/d210b1837bd50df80c71bcf37f417555f9f6f7bc',
    },
  ]}
/>
...
There are plenty of ways to host a web application in 2026. Managed Kubernetes clusters, serverless functions, PaaS providers with generous free tiers, you name it. Here's a rough map of your hosting options:
...
You don't need any of that right now. You're learning how deployment works, not architecting for millions of users. A single VPS with Docker Compose is more than enough, and when the time comes to scale, you'll understand the fundamentals well enough to know _what_ you're scaling away from.
...
The important thing is that **Coolify only deploys if GitHub Actions passes**. This prevents broken code from reaching production. GitHub Actions handles the heavy lifting of compiling and testing across all three language implementations, while Coolify focuses purely on the deployment orchestration.

## Alternative: Docker registry approach

The architecture above has Coolify building Docker images on the VPS itself. There's another common pattern where GitHub Actions builds the images, pushes them to a container registry (like [GitHub Container Registry](https://docs.github.com/en/packages/working-with-a-github-packages-registry/working-with-the-container-registry), [Docker Hub](https://hub.docker.com/), or [AWS ECR](https://aws.amazon.com/es/ecr/)), and Coolify just pulls the pre-built images:
...
| ----------------------- | -------------------------------------------------------------- | ----------------------------------------------------------------------- |
| **VPS resource usage**  | Higher: compiling happens on your server                       | Lower: your VPS only pulls and runs images                              |
| **CI minutes**          | Lower: CI only runs tests                                      | Higher: CI also builds and pushes Docker images                         |
| **Pipeline complexity** | Simpler: fewer moving parts, no registry credentials to manage | More setup: registry auth, image tagging strategy, cleanup policies     |
| **Build consistency**   | Builds happen on VPS hardware, which may differ from CI        | Images are identical everywhere, built once in a controlled environment |
| **Deployment speed**    | Slower: Coolify compiles from source each time                 | Faster: Coolify just pulls a ready-to-run image                         |
| **Rollbacks**           | Requires a rebuild from a previous commit                      | Pull a previous image tag, nearly instant                               |
| **Debugging builds**    | Harder: build logs are in Coolify, separate from CI            | Easier: everything is in one place (GitHub Actions)                     |

<Admonition type="tip" title="When to Consider a Registry">
  If your VPS is resource-constrained (1-2 GB RAM), building Java applications
  with Gradle on it can be painful. Moving the build to GitHub Actions (which
  has 7 GB RAM on free runners) and pushing pre-built images to a registry is a
  practical solution.
</Admonition>
...
https://raw.githubusercontent.com/franBec/springboot-demo-projects/d210b1837bd50df80c71bcf37f417555f9f6f7bc/docker-compose.yml
```
...
<Tabs groupId="language" queryString>
    <TabItem value="java" label="Java" default>
        ...
        ```Dockerfile reference title="spring_java/Dockerfile"
        https://raw.githubusercontent.com/franBec/springboot-demo-projects/d210b1837bd50df80c71bcf37f417555f9f6f7bc/spring_java/Dockerfile
        ```

    </TabItem>
    <TabItem value="kotlin" label="Kotlin">

        ```Dockerfile reference title="spring_kotlin/Dockerfile"
        https://raw.githubusercontent.com/franBec/springboot-demo-projects/d210b1837bd50df80c71bcf37f417555f9f6f7bc/spring_kotlin/Dockerfile
        ```

    </TabItem>
    <TabItem value="groovy" label="Groovy">

        ```Dockerfile reference title="spring_groovy/Dockerfile"
        https://raw.githubusercontent.com/franBec/springboot-demo-projects/d210b1837bd50df80c71bcf37f417555f9f6f7bc/spring_groovy/Dockerfile
        ```
...
<Tabs groupId="language" queryString>
    <TabItem value="java" label="Java" default>
        ...
        ```groovy reference title="spring_java/settings-docker.gradle"
        https://raw.githubusercontent.com/franBec/springboot-demo-projects/d210b1837bd50df80c71bcf37f417555f9f6f7bc/spring_java/settings-docker.gradle
        ```

    </TabItem>
    <TabItem value="kotlin" label="Kotlin">

        ```groovy reference title="spring_kotlin/settings-docker.gradle"
        https://raw.githubusercontent.com/franBec/springboot-demo-projects/d210b1837bd50df80c71bcf37f417555f9f6f7bc/spring_kotlin/settings-docker.gradle
        ```

    </TabItem>
    <TabItem value="groovy" label="Groovy">

        ```groovy reference title="spring_groovy/settings-docker.gradle"
        https://raw.githubusercontent.com/franBec/springboot-demo-projects/d210b1837bd50df80c71bcf37f417555f9f6f7bc/spring_groovy/settings-docker.gradle
        ```
...
```gitignore reference title=".gitignore"
https://raw.githubusercontent.com/franBec/springboot-demo-projects/d210b1837bd50df80c71bcf37f417555f9f6f7bc/.gitignore#L23
```
...
```yml reference title=".github/workflows/ci-cd.yml"
https://raw.githubusercontent.com/franBec/springboot-demo-projects/d210b1837bd50df80c71bcf37f417555f9f6f7bc/.github/workflows/ci-cd.yml
```
...
1. **Domains**: Add your custom domains:
   ...
   <Admonition type="caution" title="Port Configuration">
     **Important**: Each service is configured to expose port 8080. Coolify's
     reverse proxy will handle routing traffic to each container based on the
     domain name. If you run into routing issues, check [Coolify's
