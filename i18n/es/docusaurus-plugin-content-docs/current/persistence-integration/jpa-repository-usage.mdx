...
El archivo `sakila-schema.sql` fue agregado durante [ingeniería inversa](./reverse-engineering-jpa-entities) y dobla como el DDL que H2 carga en startup:
...
Este es el mismo `ControllerAdvice` del [setup de normalización de errores](/es/optional-but-recommended-features/normalize-errors), con un nuevo `@ExceptionHandler` agregado. El puerto secundario se mantiene limpio de lógica HTTP, y el formato de respuesta de error se mantiene consistente a través de toda la API.
