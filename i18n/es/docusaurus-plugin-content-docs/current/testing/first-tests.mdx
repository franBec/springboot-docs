...
| 🟣      | `@ContextConfiguration` | Contexto parcial (solo beans específicos) [Usado en el proyecto GROOVY] |
...
<Admonition type="info" title="Ejemplo específico de Groovy">
  Esta sección aplica principalmente al proyecto Groovy ya que es el único donde
  ...
  necesitás testear componentes con dependencias parciales o cuando mockeamos
  ciertos beans mientras testeamos otros.
</Admonition>

El objetivo de este test es demostrar cómo testear la funcionalidad del mapper con un contexto de Spring parcialmente inicializado. Queremos verificar que nuestro mapper pueda manejar casos extremos y entradas inválidas correctamente sin necesidad del contexto completo de la aplicación.
