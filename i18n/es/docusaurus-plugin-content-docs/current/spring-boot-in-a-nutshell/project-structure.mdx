...
- `test/`: Preconfigurado para tests JUnit (Más sobre esto en la [sección de Unit Testing](/testing/about-testing)). Tiene una clase de test esqueleto `SpringJavaApplication.java` que verifica que el contexto de la app carga.
...
2. Obtiene la información de películas de una base de datos [Sakila](/es/prior-recommended-knowledge/sakila-sample-database) [H2](https://h2database.com/html/main.html).
...
Este documento cubrió la regla de la main class, la estructura por defecto que genera Spring Boot, y los patrones arquitectónicos comunes como Clean Architecture y Hexagonal Architecture. El siguiente paso es generar un proyecto real usando Spring Initializr.
