...
Entender la diferencia entre errores y excepciones te ayuda a escribir aplicaciones Java más robustas. Acordate de esto:

- **Errores** — para problemas graves a nivel del sistema que no deberías intentar capturar
- **Excepciones de tiempo de ejecución** — para errores de programación que deberían corregirse en el código
- **Excepciones verificadas** — para condiciones esperadas que el código que llama debería manejar

En Spring Boot, vas a trabajar principalmente con excepciones de tiempo de ejecución. El framework envuelve muchas excepciones verificadas (como `IOException` de las llamadas HTTP) en excepciones de tiempo de ejecución, lo que te da código más limpio sin bloques try-catch excesivos.
